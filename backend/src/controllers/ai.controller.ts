...
export async function getMissionReport(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!missionRes.rows.length) throw new AppError('Mission introuvable', 404);
    ...
    const mediaRes = await db.query(
      `SELECT id, phase, type, quality_score, anomalies, ai_analysis, created_at
       FROM media
       WHERE mission_id = $1
    ...
    // Score by phase
    const byPhase: Record<string, { count: number; avgScore: number; anomalies: any[] }> = {};
    const allAnomalies: any[] = [];

    for (const m of media) {
      const phase = m.phase || 'unknown';
      if (!byPhase[phase]) byPhase[phase] = { count: 0, avgScore: 0, anomalies: [] };
      byPhase[phase].count++;
      if (m.quality_score) byPhase[phase].avgScore += parseFloat(m.quality_score);

      const anomalies = Array.isArray(m.anomalies) ? m.anomalies : [];
      byPhase[phase].anomalies.push(...anomalies);
      allAnomalies.push(...anomalies.map((a: any) => ({ ...a, phase, mediaId: m.id })));
    }
    ...
    const issues = highAnomalies.map((a: any) =>
      `[${a.phase?.toUpperCase() ?? ''}] ${a.description ?? a.type} (confiance: ${Math.round((a.confidence ?? 0) * 100)}%)`
    );

    let recommendation = 'Qualité satisfaisante.';
    if (overallScore !== null) {
      if (overallScore < 5) recommendation = 'Score insuffisant — une vérification sur site est recommandée.';
      else if (overallScore < 7) recommendation = 'Quelques points à améliorer, suivre les anomalies identifiées.';
      else if (overallScore >= 9) recommendation = 'Excellent travail, aucune action requise.';
    }
    ...
    try {
      ...
      if (hasAnalyzed) {
        aiEnhancement = await Promise.race([
          AIService.generateMissionQualityReport(missionId),
...
export async function getAgentAIScore(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const agentRes = await db.query(
      'SELECT id, first_name, last_name, role, quality_score FROM users WHERE id = $1',
      [agentId]
    );
    if (!agentRes.rows.length) throw new AppError('Agent introuvable', 404);
    ...
    const avgScore = scored.length
      ? Math.round(scored.reduce((s: number, m: any) => s + parseFloat(m.quality_score), 0) / scored.length * 10) / 10
      : null;
    ...
    const avgRecent = recent.filter(m => m.quality_score).length
      ? recent.filter(m => m.quality_score).reduce((s: number, m: any) => s + parseFloat(m.quality_score), 0) / recent.filter(m => m.quality_score).length
      : null;
    const avgOlder = older.filter(m => m.quality_score).length
      ? older.filter(m => m.quality_score).reduce((s: number, m: any) => s + parseFloat(m.quality_score), 0) / older.filter(m => m.quality_score).length
      : null;
    ...
    const totalAnomaly = missions.reduce((s: number, m: any) => s + parseInt(m.anomaly_media_count), 0);
    const totalMedia   = missions.reduce((s: number, m: any) => s + parseInt(m.media_count), 0);
